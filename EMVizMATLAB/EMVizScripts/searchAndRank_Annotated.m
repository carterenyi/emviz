...
if nargin==4 % number of arguments into the function
    ...
    % other search parameters delgap, simmin, etc should be added here
    ...
    degrees=2;
    cmax=50;
    %insert defaults for other search parameters
end

rawnmat=nmat; %store the unaltered nmat in 'rawnmat'

%% Create Contour Level ('CL') Array to search
pitches=nmat(:,4); %create an array of pitches
pitches=pitches'; %transpose from vertical to horizontal
contcom=CT_makeContCom(pitches,degrees); % create a continuous contour matrix
CL=nansum(contcom); % sum the columns of the matrix into contour levels, ignoring nans

%% Create Durational Contour Level ('DCL') Array to search
onsets=nmat(:,6); %create an array of onsets in seconds
durs=onsets(2:end)-onsets(1:end-1); %convert to interonset durations by subtracting n-1 from n;
durs(end+1)=nmat(end,7); %add the last duration to the end so that there are values for all events
contcom=CT_makeContCom(durs,degrees); % create a continuous contour matrix
DCL=nansum(contcom); % sum the columns of the matrix into contour levels, ignoring nans

%% create a searchable matrix (2 x number of events) based on type input argument
if type==1
    CL=[CL;CL]; %only pitch information
elseif type==2
    CL=[DCL;DCL]; %only durational information
elseif type==3
    CL=[CL;DCL]; %combining pitch and durational information
end

%% Search the matrix
%create a segment pool and find repeating segments (strings)
for n=cmin:cmax % for loop
